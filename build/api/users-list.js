...
const validateSupabaseEnv = (supabaseUrl, serviceKey) => {
    ...
    if (serviceKey.startsWith('sb_publishable_'))
        return 'SUPABASE_SERVICE_ROLE_KEY is wrong. Use the service role key.';
    if (/\s/.test(serviceKey))
        return 'SUPABASE_SERVICE_ROLE_KEY is invalid. It contains whitespace/new lines.';
    return '';
};
...
const getCallerRole = async (base, pgCandidates, userId) => {
    ...
    if (!r.ok || !Array.isArray(r.json) || r.json.length === 0)
        return '';
    const entry = r.json[0];
    return safeString(entry.role).toLowerCase();
};
const isInternalRole = (role) => ['ceo', 'sales', 'ops', 'staff', 'agency_staff'].includes(role);
export default async function handler(req, res) {
    try {
        ...
        if (!who.ok || !who.json || typeof who.json !== 'object' || typeof who.json.id !== 'string') {
            res.status(401).json({ error: 'Invalid token' });
            return;
        }
        const callerId = safeString(who.json.id);
        const appMeta = who.json.app_metadata;
        const callerRole = typeof appMeta === 'object' && appMeta !== null && typeof appMeta.role === 'string'
            ? safeString(appMeta.role).toLowerCase()
        ...
        const users = response.json.map((row) => normalizeUser(row)).filter(Boolean);
