...
export async function runSlaSweep() {
    ...
    if (!state)
        return { ok: false, checked: 0, penalized: 0, error: 'No shared state found' };
    ...
    for (const a0 of applications) {
        ...
        if (!pipeline || getString(pipeline, 'status') !== 'processing')
            continue;
        ...
        const group = (['fast', 'medium', 'slow', 'none'].includes(groupRaw) ? groupRaw : 'none');
    ...
    if (!newEntries.length)
        return { ok: true, checked, penalized: 0 };
    const nextState = {
        ...state,
    ...
    if (!upserted.ok)
        return { ok: false, checked, penalized: 0, error: `Failed to save: ${upserted.text?.slice(0, 200)}` };
