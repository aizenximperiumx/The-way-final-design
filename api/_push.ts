...
export type PushTokenEntry = { token: string; platform?: string; at?: string };
export type PushTokensMap = Record<string, PushTokenEntry[]>;
...
export const pushConfigured = (): boolean => getServiceAccount() !== null;
...
async function getAccessToken(sa: ServiceAccount): Promise<string | null> {
  ...
  try {
    ...
    const json = (await resp.json().catch(() => null)) as { access_token?: string; expires_in?: number } | null;
...
export type PushResult = { sent: number; deadTokens: string[] };
...
export async function sendPushToUsers(
  tokensMap: PushTokensMap | null | undefined,
  userIds: string[],
  message: PushMessage,
): Promise<PushResult> {
  const sa = getServiceAccount();
  if (!sa || !tokensMap || userIds.length === 0) return { sent: 0, deadTokens: [] };
  const targets: string[] = [];
  for (const uid of userIds) {
    for (const entry of tokensMap[uid] ?? []) {
      if (entry && typeof entry.token === 'string' && entry.token) targets.push(entry.token);
    }
  }
  if (targets.length === 0) return { sent: 0, deadTokens: [] };

  const access = await getAccessToken(sa);
  if (!access) return { sent: 0, deadTokens: [] };
...
export function prunePushTokens(tokensMap: PushTokensMap, deadTokens: string[]): PushTokensMap {
  if (!deadTokens.length) return tokensMap;
  const dead = new Set(deadTokens);
  const out: PushTokensMap = {};
  for (const [uid, entries] of Object.entries(tokensMap)) {
    const kept = (entries ?? []).filter(e => e && !dead.has(e.token));
    if (kept.length) out[uid] = kept;
  }
  return out;
}
