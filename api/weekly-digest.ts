...
import { DEFAULT_SLA_GROUPS, STAGE_LABELS, getSlaWindow, type SlaGroup } from './sla-evaluate.js';
...
export type DigestResult = { ok: boolean; sent: number; error?: string };

export async function runWeeklyDigest(force = false): Promise<DigestResult> {
  const supabaseUrl = String(process.env.SUPABASE_URL || '').trim();
  const serviceKey = String(process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_SERVICE_KEY || '').trim();
  const resendKey = String(process.env.RESEND_API_KEY || '').trim();
  if (!supabaseUrl || !serviceKey) return { ok: false, sent: 0, error: 'Supabase is not configured' };
  if (!resendKey) return { ok: false, sent: 0, error: 'RESEND_API_KEY is not configured' };
  ...
  if (!state) return { ok: false, sent: 0, error: 'No shared state found' };
  ...
  if (!force) {
    ...
    if (!isMonday || !hourOk || recentlySent) return { ok: true, sent: 0 };
  }

  const since = new Date(now.getTime() - 7 * 24 * 3600_000);
  const inWeek = (iso: string) => Boolean(iso) && new Date(iso).getTime() >= since.getTime();

  const applications = Array.isArray(state.applications) ? state.applications.map(asRecord).filter(Boolean) as Record<string, unknown>[] : [];
  const ledger = Array.isArray(state.pointsLedger) ? state.pointsLedger.map(asRecord).filter(Boolean) as Record<string, unknown>[] : [];
  const requests = Array.isArray(state.documentRequests) ? state.documentRequests.map(asRecord).filter(Boolean) as Record<string, unknown>[] : [];
  ...
  for (const a of applications) {
    const p = asRecord(a.pipeline);
    if (!p || getString(p, 'status') !== 'processing') continue;
    ...
    const window_ = getSlaWindow(stage, (['fast', 'medium', 'slow', 'none'].includes(groupRaw) ? groupRaw : 'none') as SlaGroup);
    if (!window_) continue;
    const deadline = new Date(startedAt).getTime() + window_.halfHours * 3600_000;
    if (now.getTime() > deadline) overdue.push(`${getString(a, 'name')} — ${STAGE_LABELS[stage] ?? stage}`);
  }
  ...
  for (const e of weekLedger) {
    ...
    if (uid) perUser.set(uid, (perUser.get(uid) ?? 0) + delta);
  }
  ...
  const profiles = Array.isArray(profilesResp.json) ? profilesResp.json.map(asRecord).filter(Boolean) as Record<string, unknown>[] : [];
  const nameOf = (id: string) => getString(profiles.find(p => getString(p, 'id') === id) ?? null, 'name') || 'Unknown';
  ...
  const introLines = [
    ...
    `Ratings this week: ${weekRatings.length}${weekRatings.length ? ` (${weekRatings.map(x => `${getString(x.r, 'stars') || (x.r as Record<string, unknown>).stars}` + '★').join(', ')})` : ''}`,
  ];
  const outroParts: string[] = [];
  if (leaders.length) outroParts.push(`Points this week — ${leaders.join(' · ')}.`);
  if (overdue.length) outroParts.push(`Overdue: ${overdue.slice(0, 6).join('; ')}${overdue.length > 6 ? ` and ${overdue.length - 6} more` : ''}.`);
  ...
  for (const id of ceoIds) {
    ...
    const email = (u.ok && u.json && typeof u.json === 'object') ? getString(u.json as Record<string, unknown>, 'email') : '';
    if (!email) continue;
  ...
  if (sent > 0) console.log(`Weekly digest sent to ${sent} CEO account(s)`);
  return { ok: true, sent };
}
...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  try {
    ...
    if (role !== 'ceo') { res.status(403).json({ error: 'Forbidden' }); return; }
