...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  try {
    ...
    if (!deviceToken) { res.status(400).json({ error: 'Missing token' }); return; }
    ...
    const rawState = Array.isArray(stateResp.json) && stateResp.json[0] ? (stateResp.json[0].state as Record<string, unknown>) : null;
    ...
    const tokensMap = (rawState.pushTokens && typeof rawState.pushTokens === 'object')
      ? { ...(rawState.pushTokens as Record<string, Array<{ token: string; platform?: string; at?: string }>>) }
      : {};
