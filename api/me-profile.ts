...
const validateSupabaseEnv = (supabaseUrl?: string, serviceKey?: string) => {
  ...
  if (serviceKey.startsWith('sb_publishable_')) return 'SUPABASE_SERVICE_ROLE_KEY is wrong — use the JWT service role key';
  if (/^https?:\/\//i.test(serviceKey)) return 'SUPABASE_SERVICE_ROLE_KEY is invalid';
  if (/\s/.test(serviceKey)) return 'SUPABASE_SERVICE_ROLE_KEY contains whitespace';
  return '';
};
...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  try {
    ...
    if (!who.ok || !who.json || typeof (who.json as Record<string, unknown>).id !== 'string') {
      res.status(401).json({ error: 'Invalid or expired session token' });
      return;
    }

    const authUser = who.json as Record<string, unknown>;
    const userId = safeString(authUser.id);
    const email = safeString(authUser.email);

    // Read profile row from database
    const profileResp = await fetchPostgrest(
      pgCandidates,
      `${base}/rest/v1/profiles?id=eq.${encodeURIComponent(userId)}&select=id,username,role,name,email,phone,createdAt,points,staffUniversities,assignedUniversityId,passportExpiry,visaExpiry,residenceExpiry&limit=1`,
      { method: 'GET' }
    );
    ...
    const row = profileResp.json[0] as Record<string, unknown>;
    res.status(200).json({
      user: {
        id: userId,
